...
public class ClearEditText extends EditText implements View.OnFocusChangeListener, TextWatcher {
    ...
    private final int clearIcon = R.drawable.jrmf_b_btn_close;// 删除按钮ID
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			if (getCompoundDrawables().length > 0) {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    float left = LanguageUtil.isAr(getContext())?getWidth() - event.getX():event.getX();
					touchable = left > (getWidth() - getTotalPaddingEnd()) && left < (getWidth() - getPaddingEnd());
				} else {
    ...
    public void setClearIconVisible(boolean visible) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1){
            setCompoundDrawablesRelative(null, null, right, null);
        }else{
            setCompoundDrawables(null, null, right, null);
        }
        setCompoundDrawablePadding(ScreenUtil.dip2px(getContext(), 7));
    }
