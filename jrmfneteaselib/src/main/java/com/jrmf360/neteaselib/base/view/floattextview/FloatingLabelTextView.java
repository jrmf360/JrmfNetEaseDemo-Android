...
public class FloatingLabelTextView extends FrameLayout implements TextWatcher, View.OnFocusChangeListener {
    ...
    public FloatingLabelTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.Jrmf_b_FloatingLabelTextView, 0, 0);
        try {
            mValidationMessage = a.getString(R.styleable.Jrmf_b_FloatingLabelTextView_validationMessage);
            mHintText = a.getString(R.styleable.Jrmf_b_FloatingLabelTextView_hintText);
            mAllowEmpty = a.getBoolean(R.styleable.Jrmf_b_FloatingLabelTextView_allowEmpty, true);
            mValidatorType = a.getInt(R.styleable.Jrmf_b_FloatingLabelTextView_validatorType, -1);
            mMainTextSize = a.getInt(R.styleable.Jrmf_b_FloatingLabelTextView_textSize, 18);
        } finally {
            a.recycle();
        }
        init();
...
//        mTextViewHintTop.setTextColor(mIsValid ? green : red);
    ...
    public void setText(String mes) {
        mEditText.setText(mes);
    }

    public void setHint(String mes) {
        mEditText.setHint(mes);
    }
