...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.Jrmf_b_GridPasswordView, defStyleAttr, 0);

        mTextColor = ta.getColorStateList(R.styleable.Jrmf_b_GridPasswordView_gpvTextColor);
        if (mTextColor == null)
            mTextColor = ColorStateList.valueOf(getResources().getColor(android.R.color.primary_text_light));
        int textSize = ta.getDimensionPixelSize(R.styleable.Jrmf_b_GridPasswordView_gpvTextSize, -1);
        if (textSize != -1) {
            this.mTextSize = (int) ScreenUtil.px2Sp(textSize, context);
        }

        mLineWidth = (int) ta.getDimension(R.styleable.Jrmf_b_GridPasswordView_gpvLineWidth, ScreenUtil.dp2px(context, 1));
        mLineColor = ta.getColor(R.styleable.Jrmf_b_GridPasswordView_gpvLineColor, DEFAULT_LINECOLOR);
        mGridColor = ta.getColor(R.styleable.Jrmf_b_GridPasswordView_gpvGridColor, DEFAULT_GRIDCOLOR);
        mLineDrawable = ta.getDrawable(R.styleable.Jrmf_b_GridPasswordView_gpvLineColor);
        if (mLineDrawable == null)
            mLineDrawable = new ColorDrawable(mLineColor);
        mOuterLineDrawable = generateBackgroundDrawable();

        mPasswordLength = ta.getInt(R.styleable.Jrmf_b_GridPasswordView_gpvPasswordLength, DEFAULT_PASSWORDLENGTH);
        mPasswordTransformation = ta.getString(R.styleable.Jrmf_b_GridPasswordView_gpvPasswordTransformation);
        if (TextUtils.isEmpty(mPasswordTransformation))
            mPasswordTransformation = DEFAULT_TRANSFORMATION;


        mPasswordType = ta.getInt(R.styleable.Jrmf_b_GridPasswordView_gpvPasswordType, 0);
    ...
    private void inflaterViews(Context context) {
        ...
        View rootView = inflater.inflate(R.layout.jrmf_b_gridpasswordview, this);

        mInputView = (ImeDelBugFixedEditText) rootView.findViewById(R.id.idbet_inputview);
        ...
        while (index < mPasswordLength) {
            View dividerView = inflater.inflate(R.layout.jrmf_b_divider, null);
            ...
            TextView textView = (TextView) inflater.inflate(R.layout.jrmf_b_textview, null);
