...
import com.jrmf360.neteaselib.wallet.http.HttpUrl;
...
public class PwdH5Activity extends BaseActivity {
	 ...
	 * @param fromActivity
	 * @param index
	 *            0 设置密码 1 修改密码 2 忘记密码 3 设置免密 4 取消免密 5 提现跳转到H5校验密码
	 */
	public static void intentForResult(Activity fromActivity, int index, String url, int reqCode) {
		Intent intent = new Intent(fromActivity, PwdH5Activity.class);
		Bundle bundle = new Bundle();
		bundle.putInt("index", index);
		bundle.putString("url", url);
		intent.putExtras(bundle);
		fromActivity.startActivityForResult(intent, reqCode);
	}
	 ...
	 * @param fromActivity
	 * @param index
	 *            0 设置密码 1 修改密码 2 忘记密码 3 设置免密 4 取消免密 5 提现 6 用户协议
	 */
	public static void intent(Activity fromActivity, int index, int fromKey) {
		Intent intent = new Intent(fromActivity, PwdH5Activity.class);
		Bundle bundle = new Bundle();
		bundle.putInt("index", index);
	...
	@Override public int getLayoutId() {
		return R.layout.jrmf_w_activity_pwdh5;
	}
	...
	@Override protected void initData(Bundle bundle) {
		if (bundle != null) {
			...
			if (fromKey == Constants.FROM_ADD_CARD_SEC && index == 6){//用户协议
				titleBar.setTitle(getString(R.string.jrmf_w_user_protocol));
				DialogDisplay.getInstance().dialogLoading(this,getString(R.string.jrmf_w_loading));
				webView.loadUrl(HttpUrl.H5_PROTOCOL_URL);
				return;
			}
			...
			if (index == 5) {//提现
				...
				String phoneNo = "";
				int binding = 0;
				if (fromKey == Constants.FROM_ADD_CARD_SEC && index == 0) {// 从添加银行卡第二步跳转过来-设置密码
					binding = 1;
				}
				WalletHttpManager.getUrl(context, index, phoneNo, binding, new OkHttpModelCallBack<UrlModel>() {
	...
	private void setWebViewClient() {
		...
		webView.setWebViewClient(new WebViewClient() {
			...
			@Override public boolean shouldOverrideUrlLoading(WebView view, String url) {
				...
				LogUtil.e(url);
				if (url.contains("resultNotify.shtml") || url.contains("redeemNotify.shtml")) {
					// 乌行的url，不加载
					DialogDisplay.getInstance().dialogLoading(context,getString(R.string.jrmf_w_loading));
				}
				if (url.contains("result.shtml")) {
					...
					if (h5ResultBean != null) {
						if ("SUCCESS".equals(h5ResultBean.resultCode)){//成功
							if ("U00003".equals(h5ResultBean.transCode)) {
								// 设置密码成功-保存支付密码
								UserInfoManager.getInstance().setHasPwd(true);
								// 刷新对应的页面
								refreshPage();
								ToastUtil.showToast(context, getString(R.string.jrmf_w_set_pwd_suc));
								finish();
							} else if ("U00004".equals(h5ResultBean.transCode)) {
								// 修改密码-成功
								ToastUtil.showToast(context, getString(R.string.jrmf_w_update_pwd_suc));
								finish();
							} else if ("U00005".equals(h5ResultBean.transCode)) {
								// 重置密码-成功
								ToastUtil.showToast(context, getString(R.string.jrmf_w_reset_pwd_suc));
								finish();
							} else if ("U00009".equals(h5ResultBean.transCode)) {
								// 免密协议签订
								ToastUtil.showToast(context, getString(R.string.jrmf_w_no_pwd_suc));
								finish();
							} else if ("T00003".equals(h5ResultBean.transCode)) {
								// 提现
								setResult(RESULT_OK);
								finish();
							}
							...
							if ("T00003".equals(h5ResultBean.transCode)){//提现失败关闭窗口
								finish();
							}
						}
					} else {
						// 返回的对象为null
						ToastUtil.showToast(context,getString(R.string.jrmf_w_deposit_h5_notify_error));
						LogUtil.e("生成ResultBean失败：" + url);
					}
				} else {
					view.loadUrl(url);
				}
	...
	private void refreshPage() {
		if (fromKey == Constants.FROM_SECURE_SETTING) {
			// 安全设置页面
			SecureSettingActivity settingActivity = CusActivityManager.getInstance().findActivity(SecureSettingActivity.class);
